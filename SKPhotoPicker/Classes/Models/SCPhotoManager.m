...
+ (SCPhotoManager *)sharedInstance {
    
    static SCPhotoManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[SCPhotoManager alloc] init];
...
- (PHImageRequestID)requestCoverImageForAsset:(PHAsset *)asset
    ...
    if (photoWidth < kScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        ...
        CGFloat kScreenScale = 1.7;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * kScreenScale;
        CGFloat pixelHeight = pixelWidth / aspectRatio;
        imageSize = CGSizeMake(pixelWidth, pixelHeight);
    }
...
-(SCAlbumModel *)albumModelWith:(PHAssetCollection *)collection assetResult:(PHFetchResult<PHAsset *> *) assetResult{
    ...
    NSLog(@"SCAlbumModel localIdentifier = %@",collection.localIdentifier);
    ...
    NSMutableArray<SCPhotoModel *> *items = [NSMutableArray array];
...
// 获取 相册列表数据
- (void)fetchAlbumsListComplete:(void (^)(NSArray< SCAlbumModel *> *))complete {
...
//    PHFetchResult <PHAssetCollection *> *userCustomAlbums = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeAlbum subtype:PHAssetCollectionSubtypeSmartAlbumUserLibrary options:nil]; // 用户自己创建的相册 “测试相册3”
    ...
    NSMutableArray<SCAlbumModel *> *allAlbumsArray = [NSMutableArray array];
    ...
    for (PHFetchResult *result in allAlbums) {
        for (PHAssetCollection *collection in result) {
            ...
            if ([self isCameraRollAlbum:collection.localizedTitle]) {
                SCAlbumModel *cameraRollAlbum = [self albumModelWith:collection assetResult:assetResult] ;
...
- (void)fetchcameraRollAlbumAssetComplete:(void (^)(NSArray<SCPhotoModel *> *))complete {
    
    [self fetchAlbumsAllowSelectVideo:self.allowSelectVideo allowSelectImage:self.allowSelectImage complete:complete];
}

- (void)fetchAlbumsAllowSelectVideo:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage complete:(void (^)(NSArray<SCPhotoModel *> *))complete {
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection *  _Nonnull collection, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSMutableArray<SCPhotoModel *> *userLibraryPhotos = [NSMutableArray array];
        if (!result.count) {
...
+ (PHAuthorizationStatus)photosAuthorized:(void(^)(PHAuthorizationStatus))handler
{
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
        ...
        if (@available(iOS 8,*))
        {
            ...
            // 在iOS11之后，添加图片到相册（写权限），需要用户授权，需要添加NSPhotoLibraryAddUsageDescription。
...
@implementation UIImage (SCPhotoManager)

+ (UIImage *)imageNamedFromSCBundle:(NSString *)name {
    UIImage *image = [UIImage imageNamed:[@"SCPhotoPicker.bundle" stringByAppendingPathComponent:name]];
    if (image) return image;
    if (!image)  image = [UIImage imageNamed:name];
    return image;
}

+ (UIImage *)animatedGIFWithSCData:(NSData *)data {
    if (!data) {
        return nil;
    }

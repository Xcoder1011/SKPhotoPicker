...
@interface SCPreviewVideoCell () <UIScrollViewDelegate>
...
@implementation SCPreviewVideoCell
...
- (void)setModel:(SCPhotoModel *)model {
    ...
    self.playBtn.hidden = model.mediaType == SCAssetMediaTypeVideo ? NO : YES;
...
- (void)playVideo:(UIButton *)sender {
    ...
    __weak typeof(self) weakself = self;
    [[SCPhotoManager sharedInstance] requestPlayerItemForVideo:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (UIButton *)playBtn {
    ...
    if (!_playBtn) {
        ...
        [_playBtn setImage:[UIImage imageNamedFromSCBundle:@"play_big_icon"] forState:UIControlStateNormal];
        [_playBtn addTarget:self action:@selector(playVideo:)];

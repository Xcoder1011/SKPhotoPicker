...
@interface SCPreviewPhotoCell () <UIScrollViewDelegate
#ifdef __IPHONE_9_0
...
@implementation SCPreviewPhotoCell
...
- (void)setModel:(SCPhotoModel *)model {
    ...
    if (model.mediaType == SCAssetMediaTypeImage) { // 图片
    ...
    } else if (model.mediaType == SCAssetMediaTypeGIF) {  // GIF
    ...
    if (_supportLivePhoto) {
        self.imageView.hidden = (model.mediaType == SCAssetMediaTypeImage || model.mediaType == SCAssetMediaTypeGIF) ? NO : YES;
        self.livePhotoView.hidden = model.mediaType == SCAssetMediaTypeLivePhoto ? NO : YES;
    }
#endif
...
- (void)startPlayAnimationView {
    ...
    if (self.model && self.model.asset && self.supportLivePhoto) {
        ...
        if (self.model.mediaType == SCAssetMediaTypeLivePhoto) { // LivePhoto
            ...
            PHImageRequestID livePhotoRequestID = [[SCPhotoManager sharedInstance] requestLivePhotoForAsset:model.asset imageSize:size completion:^(PHLivePhoto *livePhoto) {
    ...
    if (self.model && self.model.asset && self.model.mediaType == SCAssetMediaTypeGIF) {  // GIF
    
        PHImageRequestID imageRequestID = [[SCPhotoManager sharedInstance] requestGIFForAsset:self.model.asset completion:^(NSData *imageData, UIImageOrientation orientation) {
            if ([weakself.identifier isEqualToString:weakself.model.asset.localIdentifier]) {
                dispatch_async_on_global_queue(^{
                    ...
                    if ([UIImage respondsToSelector:@selector(sd_animatedGIFWithData:)]) {
                        ...
                        image = [UIImage animatedGIFWithSCData:imageData];
                    }
                    if (image) {
...
@interface SCAlbumListPhotoCell ()
...
@implementation SCAlbumListPhotoCell
...
- (void)setModel:(SCAlbumModel *)model {
    ...
    PHImageRequestID imageRequestID = [[SCPhotoManager sharedInstance] requestImageForAsset:[model.items lastObject].asset imageSize:size completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
